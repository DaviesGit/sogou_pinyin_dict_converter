...
{
  char16 *found_2w = static_cast<char16 *>(mybsearch(&hz_char, buf_ + start_pos_[1],
...
{
  char16 *found_w = static_cast<char16 *>(mybsearch(last_hzs, buf_ + start_pos_[word_len - 1],
       ...
       pre_len++)
  {
           ...
           item_num < npre_max)
    {
      ...
      npre_items[item_num].psb =
          ngram.get_uni_psb((size_t)(w_buf - buf_ - start_pos_[word_len - 1]) / word_len + start_id_[word_len - 1]);
